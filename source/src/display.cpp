...
void Display::loop()
{
    ...
    xGecko.loop();
    if (xGecko.succeeded()) {
        if (!xGecko.getCheckError().isEmpty()) {
            showCheckError();
        ...
        } else if (m_show_check_info && !xGecko.getCheckInfo().isEmpty()) {
...
void Display::showOnScreenDebug()
{
    if (doInterval(m_last_on_screen_debug_update, ON_SCREEN_DEBUG_INTERVAL)) {
        ...
        xTft.setCursor(7, 5);
        if (xGecko.onProAPI()) {
            xTft.setTextColor(TFT_CYAN, TFT_BLACK);
            xTft.print(F("pro"));
        } else if (xGecko.onProxy()) {
        ...
        xTft.setCursor(7, 45);
        xTft.print(F("pro:"));
        xTft.print(xGecko.proApiRequestsCount());
        xTft.print(F(" 429:"));
        ...
        xTft.fillRect(5, 85, 230, 20, TFT_BLACK);
        xTft.setCursor(7, 85);
...
void Display::showWhaleTicker(String& msg)
{
    xTft.loadFont(F("NotoSans-Condensed50"));
    msg = F("WhaleTicker");
    xTft.setCursor((DISPLAY_WIDTH - xTft.textWidth(msg)) / 2, 30);
    xTft.print(msg);
    xTft.unloadFont();
}
...
void Display::showAPIInfo(String& msg)
{
    ...
    if (xSettings.isFakeGeckoServer()) {
        msg = xSettings.getGeckoServer(false, false);
...
void Display::showCheckInfo()
{
    if (m_last_screen != Screen::CHECK_INFO) {
        xTft.fillScreen(TFT_BROWN);
        xTft.setTextColor(TFT_BLACK, TFT_BROWN);
        xTft.setTextWrap(true);

        const String& msg(xGecko.getCheckInfo());
        xTft.loadFont(F("NotoSans-Regular25"));
        xTft.setCursor((DISPLAY_WIDTH - xTft.textWidth(msg)) / 2, 95);
        xTft.print(msg);
        xTft.unloadFont();
        xTft.setTextWrap(false);
...
void Display::showCheckError()
{
    if (m_last_screen != Screen::CHECK_ERROR) {
        ...
        xTft.setTextColor(TFT_WHITE, TFT_RED);
        xTft.setTextWrap(true);

        const String& msg(xGecko.getCheckError());
        xTft.loadFont(F("NotoSans-Regular25"));
        xTft.setCursor((DISPLAY_WIDTH - xTft.textWidth(msg)) / 2, 95);
        xTft.print(msg);
        xTft.unloadFont();
